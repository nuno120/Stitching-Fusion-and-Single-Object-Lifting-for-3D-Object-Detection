...
class NuScenesExplorer:
    ...
    def get_corresponding_points(self,
                            lidar_token: str,
                            camera_token1: str,
                            camera_token2: str) -> Tuple[np.ndarray, np.ndarray, Image.Image, np.ndarray, np.ndarray, Image.Image]:
        """
        Map point clouds from two lidar sensors to their respective camera images.
        :param nusc: NuScenes object.
        :param lidar_token1: Lidar sensor token for the first point cloud.
        :param lidar_token2: Lidar sensor token for the second point cloud.
        :param camera_token1: Camera token for the first camera.
        :param camera_token2: Camera token for the second camera.
        :return: Tuple of (points1, coloring1, image1, points2, coloring2, image2).
        """

        def load_pc_and_image(lidar_token: str, camera_token: str) -> Tuple[np.ndarray, np.ndarray, Image.Image]:
            cam = self.nusc.get('sample_data', camera_token)
            ...
            # Transformation steps as in your original code
            cs_record = self.nusc.get('calibrated_sensor', pointsensor['calibrated_sensor_token'])
            pc.rotate(Quaternion(cs_record['rotation']).rotation_matrix)
            pc.translate(np.array(cs_record['translation']))
        ...
        # Load data for first lidar and camera
        points1, coloring1, im1 = load_pc_and_image(lidar_token, camera_token1)

        # Load data for second lidar and camera
        points2, coloring2, im2 = load_pc_and_image(lidar_token, camera_token2)
        min_dist = 1
        mask1 = (points1[0] > 1) & (points1[0] < im1.size[0] - 1) & \
            (points1[1] > 1) & (points1[1] < im1.size[1] - 1) & \
            (coloring1 > min_dist)

        mask2 = (points2[0] > 1) & (points2[0] < im2.size[0] - 1) & \
                (points2[1] > 1) & (points2[1] < im2.size[1] - 1) & \
                (coloring2 > min_dist)

        # Combine masks to filter points that satisfy both conditions
        combined_mask = mask1 & mask2

        # Apply combined mask to filter points and colors
        new_points1 = points1[:, combined_mask].T
        new_color1 = coloring1[combined_mask]
        new_points2 = points2[:, combined_mask].T
        new_color2 = coloring2[combined_mask]

        return new_points1, new_color1, im1, new_points2, new_color2, im2
        """
        ...
        for i in range(len(points1[0])):
            ...
            if (point1[0] > 1 and point1[0] < im1.size[0] - 1 and
                point1[1] > 1 and point1[1] < im1.size[1] - 1 and coloring1[i] > min_dist):

                point2 = points2[:, i]

                if (point2[0] > 1 and point2[0] < im2.size[0] - 1 and
                    point2[1] > 1 and point2[1] < im2.size[1] - 1 and coloring2[i] > min_dist):

                    new_points1.append(point1)
                    new_points2.append(point2)
                    new_color1.append(coloring1[i])
                    new_color2.append(coloring2[i])
        ...
        """
        """
        # Filter points within image bounds for both sets simultaneously
        min_dist = 1
        ...
        # Apply combined mask to filter points and colors
        new_points1 = points1[:, combined_mask]
        new_color1 = coloring1[combined_mask]
        new_points2 = points2[:, combined_mask]
        new_color2 = coloring2[combined_mask]
